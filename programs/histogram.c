...
void histogramEqualisation(int cols, int rows, char* input_file_name, char* output_file_name)
{
	...
	for (col = 0 , row =0 ; col < 256 ; col++,row++)
	{
		printf("%d %d\n",row ,image[col]);
	}
	...
	for (i = 0; i < 256; i++) {
		...
		new_gray_level[i] =(float) round(((curr) * 255) / (total));
	}
...
//		}
		...
		write(output_file, &image[0], 256 * sizeof(unsigned char));
	}
...
int main()
{
	...
	input_file_name = "lena_gray.raw";

	// defining output file name (output image name)
	output_file_name = "final.txt";
